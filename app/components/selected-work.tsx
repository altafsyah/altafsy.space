...
export default function SelectedWork({
  ...
  item: WorkItem;
  toggle: (item: WorkItem | undefined) => void;
}) {
  return (
    <motion.section
      initial={{ opacity: 0 }}
      ...
      className="fixed top-0 left-0 bg-black/10 backdrop-blur flex justify-center items-center w-screen h-screen p-10"
    >
      <motion.div
        ...
        animate={{ scale: 1 }}
        exit={{ scale: 0.5 }}
        className="overflow-hidden bg-white rounded-xl max-w-xl w-full max-h-full"
      >
        ...
        <div className="w-full px-5 py-2 max-h-[300px] overflow-scroll text-accent">
          <div className="flex flex-col md:flex-row justify-between gap-0 md:gap-5">
            ...
            <div className="flex items-center gap-3">
              <h4 className="px-3 py-1 bg-accent/20 text-accent font-semibold rounded-full h-fit">
                {item.year ?? ""}
              </h4>

              {item.description.links.url && (
                <a
                  href={item.description.links.url}
                  target="_blank"
                  rel="noreferrer"
                >
